...
$HADOOP_MAPRED_HOME/sbin/mr-jobhistory-daemon.sh --config $HADOOP_CONF_DIR stop historyserver
echo "Stopping Yarn Daemon Node Manager with configuration in: " $YARN_CONF_DIR 
$HADOOP_YARN_HOME/sbin/yarn-daemon.sh --config $YARN_CONF_DIR stop nodemanager
...
$HADOOP_COMMON_HOME/sbin/hadoop-daemon.sh --config $HADOOP_CONF_DIR stop namenode
